...
def detection(image_path):
    ...
    if response.status_code != 200:
        log("Detection API ERROR -> using original image")
        return image

    data = response.json()

    if "predictions" not in data or len(data["predictions"]) == 0:
        log("No object detected -> using full image")
        return image
    ...
    log("Detected object size:", cropped.shape)
...
def iterative_color_match(reference, target, max_iter=10, tolerance=0.3):
    ...
    log("\nSTEP 2 : ITERATIVE COLOR MATCH (PRO)")
    ...
    for i in range(max_iter):
        ...
        log("DeltaE:", diff)
        log("Orange shift:", orange_shift)
        log("RGB shift:", analysis["rgb_diff"])

        improvement = prev_diff - diff

        # ✅ เงื่อนไขหยุดแบบฉลาด
        if abs(improvement) < tolerance and orange_shift < 0.01:
            log("Converged (color + orange stable)")
...
def processing_img(refpath, targetpath, filename_path):
    ...
    # ANALYSIS
    analysis = analyze_rgb_and_orange_shift(reference_img, result_img)
    ...
    axes[1,2].set_title("Color Diff Iteration")
    ...
    object_path=f"static/results/{filename_path}_object.jpg"
